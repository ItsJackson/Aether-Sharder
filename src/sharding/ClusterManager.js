...
class ClusterManager extends EventEmitter {
    ...
    constructor(token, mainFile, options) {
        ...
        this.name = options.name || "Aether-Sharder";
            ...
            if (message.name) {
                ...
                switch (message.name) {
                    ...
                    case "apiRequest":
                        ...
                        try {
                            response = await this.Aether.requestHandler.request(method, url, auth, body, file, _route, short);
                        } catch (err) {
                            error = {
                                code: err.code,
                                message: err.message,
    ...
    sendWebhook(type, embed) {
        ...
        if (id && token) {
            this.Aether.executeWebhook(id, token, { embeds: [embed] });
        }
    }
    ...
    printLogo() {
        ...
        console.log(
            logo({
                ...
                .emptyLine()
                .right(`Aether-sharder ${pkg.version}`)
                .emptyLine()
                .render()
        );
    }
    ...
    async calculateShards() {
        ...
        let result = await this.Aether.getBotGateway();
